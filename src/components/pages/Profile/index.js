...
function Profile () {
    ...
    const handleSubmitProfilePicture = async (file, field) => {
        ...
        formData.append('ref', 'user') // link the image to a content type
    ...
    return  (
        <div>
            <GlobalStyles/>
            <section id='profile_banner' className='jumbotron breadcumb no-bg' style={{backgroundImage: 
                `url(${ (user && user.banner && user.banner.url ? user.banner.url : './img/background/4.jpg')})`}}>
                                ...
                                ({ values, isSubmitting, isValid }) => {
                                    ...
                                    return (
                                        <Form>
                                            ...
                                            <img 
                                                src={(user && user.photo && user.photo.url) ? profileImageTemp ? profileImageTemp : ( user.photo.url) : './img/author_single/author_thumbnail.jpg'} 
                                                id="click_profile_img" 
                                                className="d-profile-img-edit img-fluid" 
                                                alt=""
                                                style={{width: '150px', height: '150px', objectFit: 'cover'}}
                                            />
                                            <input name="profile_image" accept="image/png, image/jpeg" type="file" id="upload_profile_img" onChange={(event) => {
                                ...
                                ({ values, isSubmitting, isValid }) => {
                                    ...
                                    return (
                                        <Form>
                                            ...
                                            <img 
                                                src={(user && user.banner && user.banner.url) ? profileBannerTemp ? profileBannerTemp : ( user.banner.url) : "./img/author_single/author_banner.jpg"} 
                                                id="click_banner_img" 
                                                className="d-banner-img-edit img-fluid" 
                                                alt=""
                                            />
                                            <input name="profile_banner" accept="image/png, image/jpeg" type="file" id="upload_banner_img" onChange={(event) => {
